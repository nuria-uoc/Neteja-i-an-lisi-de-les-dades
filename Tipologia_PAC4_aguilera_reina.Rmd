---
title: "PAC4 - Tipologia i cicle de vida de les dades"
...
Aquest conjunt de dades permeten realitzar tasques de classificació i regressió i per tant poden donar resposat a diferents preguntes o problemes que es plantegen:
...
L _volatile.acidity_ mitjana, es de 0.52grams. Un excès d'acidesa volàtil dona com a resultat un gust de vinagre dolent. Potser els vins que tenen una qualitat menor es troben en la cua dreta en la representació del nivell més alt d'acidesa.
...
Com a tractament de valors extrems es decideix que no s'eliminaran sino que es substituiran per la mitja del valor de cada varaible dins del grup al que pertany (qualitat de vi).
...
Desprès es crearà una matriu formada per la mitjana de les diferents variables, agruapdes per qualitat. On a les columnes es trobarà les diferents variables i les files correspondran a cada grup de qualitat del vi.
...
Després de la realització de les diferents modelitzacions per contestar les preguntes o problemes plantejats, aquestes son les conclusions:
